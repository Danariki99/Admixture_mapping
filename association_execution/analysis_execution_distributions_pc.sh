...
# Extract unique (ancestry, pheno) pairs from the hits folder
hits_folder="/private/groups/ioannidislab/smeriglio/out_cleaned_codes/vcf_files_windows/ukbb/fine_mapping_ancestries_PCA_verbose"

declare -A pairs
for dir in $(ls $hits_folder); do
    anc=$(echo $dir | cut -d'_' -f1)
    pheno=$(echo $dir | cut -d'_' -f2)
    pairs["${anc}_${pheno}"]=1
done
...
covar_file="/private/groups/ioannidislab/smeriglio/out_cleaned_codes/covar_file/${dataset}/ukb24983_GWAS_covar_filtered_proportions_pc.phe"
...
for key in "${!pairs[@]}"
do
    anc=$(echo $key | cut -d'_' -f1)
    pheno=$(echo $key | cut -d'_' -f2)

    vcf_file="/private/groups/ioannidislab/smeriglio/out_cleaned_codes/vcf_files/${dataset}/ancestry_${anc}.vcf"
    current_pheno_file="$pheno_folder/${pheno}.phe"

    echo "Processing ${anc} ${pheno}"

    output_dir="/private/groups/ioannidislab/smeriglio/out_cleaned_codes/output_distributions_pc/${dataset}/output_ancestry_${anc}/$pheno"
    mkdir -p $output_dir
    output_file="$output_dir/output"

    command_to_run="/private/home/rsmerigl/plink2 --vcf $vcf_file --pheno $current_pheno_file --glm firth-fallback hide-covar --ci 0.95 --adjust --covar $covar_file --covar-variance-standardize --keep $keep_file --out $output_file --covar-col-nums 2-7,9-20"

    sbatch_file="$sbatch_dir/${anc}_${pheno}_$job_counter.sh"
    echo "#!/bin/bash" > $sbatch_file
    echo "#SBATCH --partition=long" >> $sbatch_file
    echo "#SBATCH --job-name=${sbatch_base}_${anc}_${pheno}_$job_counter" >> $sbatch_file
    echo "#SBATCH --output=$sbatch_dir/${anc}_${pheno}_$job_counter.out" >> $sbatch_file
    echo "#SBATCH --error=$sbatch_dir/${anc}_${pheno}_$job_counter.err" >> $sbatch_file
