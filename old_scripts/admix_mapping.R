...
#local ancestry - comment out to line 136 if using only global admixture -
if(groups == 3){
  local_NAT <- read.table(paste0("./admix_files/3_groups/",prefix,".NAT.chr", chr, ".",groups,"_groups.txt.gz"),header=T, stringsAsFactors = F)
  local_AFR <-read.table(paste0("./admix_files/3_groups/",prefix,".AFR.chr", chr, ".",groups,"_groups.txt.gz"),header=T, stringsAsFactors = F)
}
if(groups == 4){
  local_NAT <- read.table(paste0("./admix_files/4_groups/",prefix,".NAT.chr", chr, ".",groups,"_groups.txt.gz"),header=T, stringsAsFactors = F)
...
if(groups == 5){
  local_NAT <- read.table(paste0("./admix_files/5_groups/", prefix, ".NAT.chr", chr, ".",groups,"_groups.txt.gz"),header=T, stringsAsFactors = F)
...
#la_id <- local_NAT$ID[!(local_NAT$ID %in% p$ID)]
#p_id <- p$ID[!(p$ID %in% local_NAT$ID)]

#local_NAT$ID[!(local_NAT$ID %in% p$ID)] <- p_id
#local_AFR$ID[!(local_AFR$ID %in% p$ID)] <- p_id

for (s in sites){
  
  #prepare pheno file combining local and global ancestry
  NAT_subset <- local_NAT[c("ID",s)]
  colnames(NAT_subset) <- c(colnames(NAT_subset)[1], paste(colnames(NAT_subset)[2],"_NAT",sep=""))
  AFR_subset <- local_AFR[c("ID",s)]
  colnames(AFR_subset) <- c(colnames(AFR_subset)[1], paste(colnames(AFR_subset)[2],"_AFR",sep=""))
  foo <- merge(p,NAT_subset,by.x="ID", by.y="ID", all.x=T)  
  ...
  #model fit
  factors<- c(paste(s,"_NAT", sep=""),paste(s,"_AFR", sep=""),"NAT","AFR","SEX", "AGE")
  fit1 <- glm(as.formula(paste(trait,"~",paste(factors, collapse = "+"))),family = "binomial", data=foo)
  fit2 <- glm(paste(trait,"~NAT+AFR+SEX+AGE"),family = "binomial", data=foo)


  if(groups >= 4){
    remove(fit1,fit2,factors)
    factors<- c(paste(s,"_NAT", sep=""),paste(s,"_AFR", sep=""),paste(s,"_EAS", sep=""),"NAT","EAS","AFR","AGE","SEX")
    fit1 <- glm(as.formula(paste(trait,"~",paste(factors, collapse = "+"))),family = "binomial", data=foo)
    fit2 <- glm(paste(trait,"~NAT+EAS+AFR+AGE+SEX"),family = "binomial", data=foo)
  }

  if(groups == 5){
    remove(fit1,fit2,factors)
    factors<- c(paste(s,"_NAT", sep=""),paste(s,"_AFR", sep=""),paste(s,"_EAS", sep=""),paste(s,"_SAS", sep=""),"NAT","EAS","AFR","SAS", "AGE","SEX")
    fit1 <- glm(as.formula(paste(trait,"~",paste(factors, collapse = "+"))),family = "binomial", data=foo)
    fit2 <- glm(paste(trait,"~NAT+EAS+AFR+SAS+AGE+SEX"),family = "binomial", data=foo)
  }
  ...
  #results from fit1
  #NAT
  results$NAT_beta[results$sites == s] <- summary(fit1)$coefficients[2,1]
  results$NAT_pvalue[results$sites == s] <- summary(fit1)$coefficients[2,4]
...
##to get more interpretable segment names:
