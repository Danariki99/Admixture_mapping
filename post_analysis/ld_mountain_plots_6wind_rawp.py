"""
"Mountain" / TAD-style plot per hit — EXTENDED (6-window), RAW-p version:
  - top   : LD matrix (r^2) as a 45-degree triangular heatmap
  - bottom: association track as a bar plot. EVERY SNP is drawn
            (height = -log10 of the RAW, un-corrected p), all bars pointing UP.
...
Identical to ld_mountain_plots_6wind_signif.py except the bar HEIGHT is the raw
-log10(p) rather than the BY-corrected value (so the shape of weak signals is
still visible in hits where BY correction flattens everything to ~1).

Output: ld_mountain_plots_6wind_rawp/{hit}_mountain.png / .pdf
"""
...
OUT_DIR  = os.path.join(os.path.dirname(__file__), 'ld_mountain_plots_6wind_rawp')
...
GW_P = 5e-8         # genome-wide threshold, drawn only as a reference guide line
...
def make_plot(hit):
    ...
    # ── RAW -log10(p) height (no multiple-testing correction) ───────────────────
    height = -np.log10(np.where((P > 0) & np.isfinite(P), P, np.nan))

    # ── figure: heatmap (top) + bar track (bottom), shared SNP-index x ──────────
