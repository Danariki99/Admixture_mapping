import os
import re
import sys
import h5py
import numpy as np
import pandas as pd

# TEST version of borzoi_sed_post_processing.py — reads the 4-fold sed.h5,
# averages across folds, and exports per-(SNP, gene) SED tables.
# Parameterized on result_folder.

if len(sys.argv) < 2:
    print("Usage: python borzoi_sed_post_processing_test.py <result_folder>")
    sys.exit(1)

result_folder = sys.argv[1]
SED_BASE = os.path.join(result_folder, 'borzoi_results', 'sed_windows')
BORZOI_DIR = os.environ.get('BORZOI_DIR', '/private/home/rsmerigl/codes/cleaned_codes/borzoi')
GTF_PATH = os.environ.get(
    'BORZOI_GTF',
    os.path.join(BORZOI_DIR, 'examples/hg38/genes/gencode41/gencode41_basic_nort.gtf'))
FOLDS = ['f3c0', 'f3c1', 'f3c2', 'f3c3']

sed_arrays = []
genes = snps = chrom = pos = ref = alt = track_labels = snp_idx = None

for fold in FOLDS:
    h5_path = os.path.join(SED_BASE, fold, 'sed.h5')
    if not os.path.exists(h5_path):
        print(f'Missing {h5_path} — run borzoi_sed_windows_test.py first. Skipping.')
        sys.exit(0)
    print(f'Reading {h5_path} ...')
    with h5py.File(h5_path, 'r') as f:
        sed = f['SED'][()].astype(np.float32)
        si  = f['si'][()]
        g   = [x.decode() for x in f['gene'][()]]
        s   = [x.decode() for x in f['snp'][()]]
        ch  = [x.decode() for x in f['chr'][()]]
        po  = f['pos'][()].tolist()
        rf  = [x.decode() for x in f['ref_allele'][()]]
        al  = [x.decode() for x in f['alt_allele'][()]]
        tl  = [x.decode() for x in f['target_labels'][()]]
    if genes is None:
        genes, snp_idx = g, si
        snps  = [s[i] for i in si]
        chrom = [ch[i] for i in si]
        pos   = [po[i] for i in si]
        ref   = [rf[i] for i in si]
        alt   = [al[i] for i in si]
        track_labels = tl
    sed_arrays.append(sed)

sed_mean = np.mean(np.stack(sed_arrays, axis=0), axis=0)
n_pairs, n_tracks = sed_mean.shape
print(f'\n{n_pairs} (SNP, gene) pairs x {n_tracks} tracks')

# ENSG -> gene symbol from the same GENCODE GTF Borzoi used
def build_gene_name_map(gtf_path, wanted_base_ids):
    name_map = {}
    if not os.path.exists(gtf_path):
        print(f'  GTF not found ({gtf_path}); keeping ENSG ids as names')
        return name_map
    with open(gtf_path) as fh:
        for line in fh:
            if '\tgene\t' not in line:
                continue
            gid = re.search(r'gene_id "([^"]+)"', line)
            gnm = re.search(r'gene_name "([^"]+)"', line)
            if gid and gnm and gid.group(1).split('.')[0] in wanted_base_ids:
                name_map[gid.group(1).split('.')[0]] = gnm.group(1)
    return name_map

wanted = {g.split('.')[0] for g in genes}
gene_name_map = build_gene_name_map(GTF_PATH, wanted)
gene_names = [gene_name_map.get(g.split('.')[0], g.split('.')[0]) for g in genes]

meta = pd.DataFrame({'snp': snps, 'gene': genes, 'gene_name': gene_names,
                     'chr': chrom, 'pos': pos, 'ref': ref, 'alt': alt})

# long table (SNP, gene, track)
long_rows = []
for j, tl in enumerate(track_labels):
    df = meta.copy(); df['track'] = tl; df['SED'] = sed_mean[:, j]
    long_rows.append(df)
long_out = os.path.join(SED_BASE, 'sed_mean_long.csv')
pd.concat(long_rows, ignore_index=True).to_csv(long_out, index=False)
print(f'  -> {long_out}')

# per-(SNP, gene) summary: strongest-effect track
summ = []
for i in range(n_pairs):
    row = sed_mean[i, :]
    j = int(np.argmax(np.abs(row)))
    summ.append({'snp': snps[i], 'gene': genes[i], 'gene_name': gene_names[i],
                 'chr': chrom[i], 'pos': pos[i], 'max_abs_SED': row[j],
                 'top_track': track_labels[j], 'mean_SED_alltracks': float(np.mean(row))})
summ_df = pd.DataFrame(summ).sort_values(
    ['snp', 'max_abs_SED'],
    key=lambda c: c.abs() if c.name == 'max_abs_SED' else c, ascending=[True, False])
summ_out = os.path.join(SED_BASE, 'sed_gene_summary.csv')
summ_df.to_csv(summ_out, index=False)
print(f'  -> {summ_out}')
