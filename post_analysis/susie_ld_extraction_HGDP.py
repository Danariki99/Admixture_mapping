...
def load_zscores_positions(hit_label):
    """Load z-score positions (hg19) from fine-mapping folder."""
    rows = []
...
# Get hit regions from covar files (one per significant window)
hit_regions = defaultdict(lambda: {'starts': [], 'ends': [], 'ancestry': None, 'chr_val': None})

for fname in sorted(os.listdir(covar_folder)):
