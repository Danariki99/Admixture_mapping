"""
Prepares SuSiE inputs for each hit (HGDP+1kG ancestry-specific LD):
  Same logic as susie_prepare_inputs.py but reads from SuSiE_ld_HGDP/
  and saves to SuSiE_inputs_HGDP/.
"""
...
susie_ld_folder     = '/private/groups/ioannidislab/smeriglio/out_cleaned_codes/vcf_files_windows/ukbb/SuSiE_ld_HGDP'
output_folder       = '/private/groups/ioannidislab/smeriglio/out_cleaned_codes/vcf_files_windows/ukbb/SuSiE_inputs_HGDP'
...
for hit_dir in sorted(os.listdir(fine_mapping_folder)):
    ...
    hit_label  = hit_dir
    vcor1_file = os.path.join(susie_ld_folder, hit_label, f'{hit_label}_ld_HGDP.phased.vcor1')
    vars_file  = os.path.join(susie_ld_folder, hit_label, f'{hit_label}_ld_HGDP.phased.vcor1.vars')

    if not os.path.exists(vcor1_file):
        print(f"{hit_label}: HGDP LD file not found, skipping")
        continue
    ...
    # Build hg19→hg38 position mapping from the BED file saved by extraction script
    ...
    # Map z-score positions hg19 → hg38 key
