...
for hit in hits_list:
    ...
    for ancestry in ancestry_list:
        ...
        if len(snps_list) != 0:
            ...
            most_significant_SNP = filtered_df.loc[filtered_df['P'].idxmin()]

            dataset = pd.read_csv(f'{current_dataset_folder}/{most_significant_SNP["ID"]}.tsv', sep='\t')
            dataset = dataset[dataset[imp_ancestry] == 1]
            

            model = pd.read_csv(f'{current_model_folder}/{most_significant_SNP["ID"]}.csv')
